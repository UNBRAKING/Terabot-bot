...
"""
    ...
    await m.reply(
        ...
        buttons=[
            ...
            [
                Button.url("Channel ", url="https://t.me/MTB_Channel"),
                Button.url("Group ", url="https://t.me/MTBC_CHAT"),
            ],
            [
                Button.url("Owner ", url="https://t.me/Pbail"),
            ],
        ],
    )
...
async def broadcast_message(m: UpdateNewMessage):
    ...
    all_users = await bot.get_participants(-1002194819681)  # Replace with your group ID
...
# ┃
# ┃ To check availabe plan do /plan in chat group @MTBC_CHAT
# ╚═════════════════⍟
# For subscription inquiries, contact @Pbail.
# """

#     # Send the welcome message
#     check_if = await is_user_on_chat(bot, "@MTB_Channel", m.peer_id)
#     if not check_if:
#         return await m.reply("Please join @MTB_Channel then send me the link again.")
...
To purchase premium, send a message to @Pbail.
"""
...
async def handle_message(m: Message):
    ...
    if not check_if:
        return await m.reply("Please join @MTB_Channel then send me the link again.")
    check_if = await is_user_on_chat(bot, "@MTBC_CHAT", m.peer_id)
    if not check_if:
        return await m.reply(
            "Please join @MTBC_CHAT then send me the link again."
        )
